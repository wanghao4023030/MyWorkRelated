...
For $i = 1 to $LoopCount step 1
	_FileWriteLog(@ScriptDir & "\log.txt", "No: " & $i & " try to print the films or reports")
	PrintFilm()
...
;~ 		while $Run_flag
			...
			While $b_PrintFinished
				...
				$s_temp_sql = "select top 1 Status from wggc.dbo.AFP_PrintTask where PatientID = (select distinct PatientID from wggc.dbo.AFP_FilmInfo where AccessionNumber = '"&$Accn&"')  and CreateTime > '" & $s_currentDateTime& "' order by SN desc"
				...
				If $s_result == "4" or $s_result == "3" Then
					...
					If $s_result == "4" Then
						_FileWriteLog(@ScriptDir & "\log.txt", "Print successfully!")
					Else
						_FileWriteLog(@ScriptDir & "\log.txt", "Print failed!")
					EndIf
