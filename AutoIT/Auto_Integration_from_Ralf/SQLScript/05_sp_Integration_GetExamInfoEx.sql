USE [WGGC]
GO
/****** Object:  StoredProcedure [dbo].[sp_Integration_GetExamInfoEx]    Script Date: 2019/8/12 17:35:05 ******/
SET ANSI_NULLS ON
GO
...
IF EXISTS(SELECT 1 FROM sysobjects WHERE id=object_id('sp_Integration_GetExamInfoEx') and xtype='P')   
BEGIN
	...
	BEGIN TRY
		...
		ELSE BEGIN
			...
			BEGIN
				...
				DECLARE @i int
				SET @i=PATINDEX('%[A-Za-z]%',@PatientName)
				IF(@i>0)
				BEGIN
					SET @WHERE = @WHERE+' AND (NameEN IN (''''' + @PatientName + '''''))'
				END
				ELSE
				BEGIN
					SET @WHERE = @WHERE+' AND (NameCN IN (''''' + @PatientName + '''''))'
				END
			END
		...
		IF(0<LEN(@WHERE))
		BEGIN
			SET @SQL = 'SELECT [PatientID],[AccessionNumber],[NameEN],[NameCN],[Gender],[Birthday],[Modality],[ModalityName],[ExamName]
				...
				FROM OPENQUERY(RISDB,''SELECT * FROM gcris2.dbo.vi_KIOSK_ExamInfo_Order WHERE 1=1' + @WHERE + ''') AS DS ORDER BY REGISTERDT DESC, EXAMDT DESC'
			EXEC (@SQL)
		END
