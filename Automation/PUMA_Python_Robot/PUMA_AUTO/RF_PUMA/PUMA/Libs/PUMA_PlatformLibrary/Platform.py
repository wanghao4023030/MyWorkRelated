...
class Platform(object):
    ...
    *******************************************************************************************************************
    ...
    def platform_worklist_fuzzy_query_by_patientID(self, str_patientID):
        ...
        The compare base data is base the wggc.dbo.afp_examinfo table and part data of API.
        ...
        '''
        Format the data from API, such datatime format
        '''
        worklistDto_list = self._worklist_queryout_data_format(worklistDto_list)

        data_from_DB = self._worklist_fuzzy_query_data_from_db(PatientID=str_patientID)

        return self._worklist_query_data_compare_API_DB(worklistDto_list, data_from_DB)
    ...
    def platform_worklist_fuzzy_query_by_accn(self, str_accn):
        ...
        data_from_DB = self._worklist_fuzzy_query_data_from_db(accessionNumber=str_accn)
    ...
    def platform_worklist_fuzzy_query_by_patientname(self, str_patient_name):
        ...
        data_from_DB = self._worklist_fuzzy_query_data_from_db(patientName=str_patient_name)
    ...
    def platform_worklist_fuzzy_query_by_patienttype(self, str_patient_type):
        ...
        data_from_DB = self._worklist_fuzzy_query_data_from_db(patientType=str_patient_type)

        if self._worklist_query_data_compare_API_DB(worklistDto_list, data_from_DB):
        ...
        if len(rows) >= 1:
            ...
            try:
                ...
                for index in range(len(worklistDto_list)):
                    temp_dict = {}
                    for key, value in worklistDto_list[index].items():
                        if key in self.worklist_query_db_column:
                            temp_dict[key] = value
                    worklistDto_list[index] = temp_dict
    ...
    def _worklist_queryout_data_format(self, worklistDto_list):
        '''
        Process the query API data from worklist query API
        ...
        try:
            ...
            return worklistDto_list
        except Exception as e:
            print("Platform._worklist_queryout_data_format: format the data from database to default data"
                  " type(datetime) failed.")
            print(type(e))
            raise Exception(e)

    def _worklist_fuzzy_query_data_from_db(self, **kwargs):
        '''
        ...
        try:
            ...
            for argk, argv in kwargs.items():
                if index < len(kwargs):
                    if argk.lower() == 'patienttype':
                        sql_string = str(argk) + " in (" + str(argv) + ") AND " + sql_string
                    else:
                        sql_string = str(argk) + " like '%" + str(argv) + "%' AND " + sql_string
                else:
                    if argk.lower() == 'patienttype':
                        sql_string = str(argk) + " in (" + str(argv) + ") "
                    else:
                        sql_string = sql_string + str(argk) + " like '%" + str(argv) + "%' "
                index += 1
            ...
            print("Platform._worklist_fuzzy_query_data_from_db: Try to get the data from database failed.")
            print(type(e))
            raise Exception(e)
        ...
        try:
            ...
            '''
            Map the HospitalDistrictType value to the data
            '''
            ...
            for index in range(len(data_from_DB)):
                ...
                #For defect 100057104 The API return value is not correct...
                #if HospitalDistrictType_code is None:
                    #data_from_DB[index]['hospitalDistrict'] = ''
                #data_from_DB[index]['departmentID'] = None

            return data_from_DB
        except Exception as e:
            print("Platform._worklist_fuzzy_query_data_from_db:"
                  "Try to organize the queried data to dict with the define column propery failed.")
            print(type(e))
            raise Exception(e)

    def _worklist_query_data_compare_API_DB(self, worklistDto_list, data_from_DB):
        '''
        ...
        try:
            if len(worklistDto_list) == len(data_from_DB):
                print("Platform._worklist_query_data_compare_API_DB: There are [%s] rows data queried out from DB."
                      % (len(data_from_DB)))
                print("Platform._worklist_query_data_compare_API_DB: There are [%s] rows data queried out from API"
                      % (len(worklistDto_list)))

                if operator.eq(worklistDto_list, data_from_DB):
                    print("Platform._worklist_query_data_compare_API_DB: The query data is correct!")
                    return True
                else:
                    for index in range(len(data_from_DB)):
                        if operator.eq(worklistDto_list[index], data_from_DB[index]) is False:
                            for key, value in dict(data_from_DB[index]).items():
                                if operator.eq(data_from_DB[index][key], worklistDto_list[index][key]) is False:
                                    print("Platform._worklist_query_data_compare_API_DB: The data is not equal!")
                                    print("Platform._worklist_query_data_compare_API_DB: From API: \n %s"
                                          % (worklistDto_list[index]))
                                    print("Platform._worklist_query_data_compare_API_DB: From database: \n %s"
                                          % (data_from_DB[index]))
                                    print(key + " : From API [" \
                                          +  str(worklistDto_list[index][key]) + "]  -   From database[" + str(data_from_DB[index][key]) + "]")

                    return False
            else:
                print("Platform._worklist_query_data_compare_API_DB: The data count from API are [%s]."
                      % (len(worklistDto_list)))
                print("Platform._worklist_query_data_compare_API_DB: The data count from database are [%s]."
                      % (len(data_from_DB)))
                print("Platform._worklist_query_data_compare_API_DB: The data count is not equaled.")
                return False
        except Exception as e:
            print("Platform._worklist_query_data_compare_API_DB: Compare the data between API and database "
                  "failed ")
            print(type(e))
            raise Exception(e)
    ...
    def platform_worklist_fuzzy_query_by_patientID_temp(self, str_patientID):
        ...
        print(self._worklist_query_data_compare_API_DB(worklistDto_list, data_from_DB))
