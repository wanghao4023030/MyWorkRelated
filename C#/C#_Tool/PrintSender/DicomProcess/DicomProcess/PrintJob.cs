...
namespace Print_SCU
{
    class PrintJob
    {
        ...
        {
            var filmBox = new FilmBox(FilmSession, null, DicomTransferSyntax.ImplicitVRLittleEndian)
            {
            ...
            foreach (var filmbox in FilmSession.BasicFilmBoxes)
            {
                ...
                filmBoxRequest.OnResponseReceived = (request, response) =>
                {
                    if (response.HasDataset)
                    {
                        ...
                        //var seq = response.Dataset.GetPrivateTag(DicomTag.ReferencedImageBoxSequence);
