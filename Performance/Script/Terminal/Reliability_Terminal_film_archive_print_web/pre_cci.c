# 1 "d:\\scripts\\releasescripts\\reliability_terminal_film_archive_print_web\\\\combined_Reliability_Terminal_film_archive_print_web.c"
# 1 "globals.h" 1
...
# 1 "d:\\scripts\\releasescripts\\reliability_terminal_film_archive_print_web\\\\combined_Reliability_Terminal_film_archive_print_web.c" 2
...
# 4 "d:\\scripts\\releasescripts\\reliability_terminal_film_archive_print_web\\\\combined_Reliability_Terminal_film_archive_print_web.c" 2
...
lr_think_time(60);
	...
	if(HTTP_RC !=200){
		lr_fail_trans_with_error("Create film test data faile, it`s not an error. The http return %d",HTTP_RC);
		return 0;
	}
	...
	ServiceResultFlag = strcmp(lr_eval_string("{CreateFilmDataResult}"),"true");
					...
					lr_fail_trans_with_error("Create film test data faile, it`s not an error. The return value is not true.");
		...
		lr_think_time(10);
			...
			tmpSQL = " select AccessionNumber from  wggc.dbo.AFP_FilmInfo  where AccessionNumber='A{TreatmentID_new}' and DeleteStatus =0 and FilmFlag =0 ";
			...
			lr_think_time(10);
				...
				if(stricmp(lr_eval_string("{TaskID}") ,"") ==0)
				{
					...
					lr_start_transaction("Fim GetTaskID_fail");
						lr_output_message("The accession number is %s,%s",lr_eval_string("A{TreatmentID_new}"),lr_eval_string("{K2Terminal}"));
				...
				if (LoopNumber == 10) {
						lr_start_transaction("Film Create_PrintTask Fail");
							lr_fail_trans_with_error("Try to get the Print task ID failed: %s :  %s",lr_eval_string("A{TreatmentID_new}"),lr_eval_string("{K2Terminal}"));
						...
						if (result==0) {
							...
							lr_end_transaction("Film PrintTask_Result_Correct", 2);
								...
								if (LoopNumber >=30) {
									...
									lr_fail_trans_with_error("The task do not finish correct: TaskID: %s  From Terminal: %s",lr_eval_string("{TaskID}"),lr_eval_string("{K2Terminal}"));
...
# 5 "d:\\scripts\\releasescripts\\reliability_terminal_film_archive_print_web\\\\combined_Reliability_Terminal_film_archive_print_web.c" 2
...
# 6 "d:\\scripts\\releasescripts\\reliability_terminal_film_archive_print_web\\\\combined_Reliability_Terminal_film_archive_print_web.c" 2
